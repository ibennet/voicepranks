"""Streaming "Minionese" gibberish voice effect (time-domain).

This is a full rewrite of an earlier STFT formant-replacement version. That
version rebuilt the spectral envelope frame by frame while keeping the
original phase, and the resulting magnitude/phase mismatch produced constant
broadband static -- the same class of "airy/phasy" artifact that got the
phase-vocoder pitch shifter rejected in `dsp/pitch.py`. So Minionese now
stays entirely in the time domain and is built on top of the clean WSOLA
pitch shifter, which means it inherits that path's clean tone instead of
fighting it.

The chain, all streaming and click-free:

1. Pitch-up: the WSOLA `PitchShifter` from `dsp/pitch.py` at +5 semitones.
   Formants ride up with pitch (the "chipmunk"/minion timbre).
...
Latency is roughly the pitch pipeline plus `JITTER_MS + GRAIN_MS` of grain
lookback (~0.2s) -- fine for a voiceprank, not a phone call.
"""
...
def _slew_limit(x: np.ndarray, max_step: float, prev: float):
    ...
    The clamp is a sequential recurrence (each output is measured against the
    previous *output*), so it can't be fully vectorized. But it only ever
    engages when some sample-to-sample step exceeds ``max_step`` -- and when no
    step does, the output is identical to the input. So we vectorize that
    common case (a single ``np.diff`` check) and fall back to the per-sample
    loop only for blocks that actually contain an offending discontinuity.
    """
    if x.size == 0:
        return x, prev

    xf = x.astype(np.float32)
    # Steps within the block, plus the step from the carried-over prev sample.
    if (np.abs(np.diff(xf)) <= max_step).all() and abs(float(xf[0]) - prev) <= max_step:
        return xf, float(xf[-1])

    y = xf.astype(np.float64)
    p = prev
...
class _PitchWobble:
    ...
    def __init__(self, sample_rate: int) -> None:
        ...
        self.depth = WOBBLE_MS / 1000.0 * self.sr           # peak excursion
        self.delay = int(np.ceil(self.depth)) + 4
        self.rates = WOBBLE_RATES
...
class _ChunkShuffle:
    ...
    def __init__(self, sample_rate: int, seed: int = 0) -> None:
        ...
        self.C = max(2, int(CHUNK_MS / 1000.0 * self.sr))
        self.K = max(1, int(SHUFFLE_K))
        self.fade = max(0, int(FADE_MS / 1000.0 * self.sr))
...
class Minionese:
    """Streaming time-domain "Minionese" gibberish effect.
    ...
    def __init__(self, sample_rate: int, seed: int = 0) -> None:
        ...
        self._wobble = _PitchWobble(self.sample_rate)
        self._scramble = _ChunkShuffle(self.sample_rate, seed=seed)
